...
$(function() {
    ...
    function adaptMenuOnResize() {
        var screenWidth = document.body.clientWidth;    
        if (screenWidth <= 786) {
            $('.navbar').bind('mousemove', onMouseOver)
            $('.navbar').bind('mouseleave', onMouseLeave)
        } else {
            $('.navbar').unbind('mousemove', onMouseOver)
            $('.navbar').unbind('mouseleave', onMouseLeave)
        }
    }
    adaptMenuOnResize();
    $(window).resize(adaptMenuOnResize);
});